...
class MainBlock(nn.Module):
    ...
    def forward(self, x):
            out = self.net(x)
            if self.residual:
                # Make sure residual matches the output shape
...
class QuartzNet(nn.Module):
    ...
    def forward(self, audio_signal):
        ...
        # BxCxT
        return self.classify(feat).permute(0, 2, 1)
...
if __name__  == "__main__":
    ...
    # Instantiate model
    model = QuartzNet(model_config=_quartznet5x5_config, feat_in=64, vocab_size=29)

    # Dummy input: batch_size=3, channels=64, time_steps=1300
    x = torch.rand(3, 64, 1300)

    # Forward pass
    y = model(x)
    print(y.shape)  # Should be (3, 29, T_out), where T_out depends on strides/dilations
